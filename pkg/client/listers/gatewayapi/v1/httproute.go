...
type HTTPRouteLister interface {
	...
	List(selector labels.Selector) (ret []*gatewayapiv1.HTTPRoute, err error)
...
type hTTPRouteLister struct {
	listers.ResourceIndexer[*gatewayapiv1.HTTPRoute]
}

// NewHTTPRouteLister returns a new HTTPRouteLister.
func NewHTTPRouteLister(indexer cache.Indexer) HTTPRouteLister {
	return &hTTPRouteLister{listers.New[*gatewayapiv1.HTTPRoute](indexer, gatewayapiv1.Resource("httproute"))}
}

// HTTPRoutes returns an object that can list and get HTTPRoutes.
func (s *hTTPRouteLister) HTTPRoutes(namespace string) HTTPRouteNamespaceLister {
	return hTTPRouteNamespaceLister{listers.NewNamespaced[*gatewayapiv1.HTTPRoute](s.ResourceIndexer, namespace)}
}
...
type HTTPRouteNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*gatewayapiv1.HTTPRoute, error)
