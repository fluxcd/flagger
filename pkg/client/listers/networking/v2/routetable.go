...
type RouteTableLister interface {
	...
	List(selector labels.Selector) (ret []*v2.RouteTable, err error)
...
// List lists all RouteTables in the indexer.
func (s *routeTableLister) List(selector labels.Selector) (ret []*v2.RouteTable, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2.RouteTable))
	})
	return ret, err
}
...
type RouteTableNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2.RouteTable, error)
...
// List lists all RouteTables in the indexer for a given namespace.
func (s routeTableNamespaceLister) List(selector labels.Selector) (ret []*v2.RouteTable, err error) {
...
// Get retrieves the RouteTable from the indexer for a given namespace and name.
func (s routeTableNamespaceLister) Get(name string) (*v2.RouteTable, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2.Resource("routetable"), name)
	}
	return obj.(*v2.RouteTable), nil
}
