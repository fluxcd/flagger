...
func (task *ConcordTask) checkStatus(ctx context.Context, instanceID string, interval time.Duration) (bool, error) {
	...
	for {
		...
		case <-tickChan:
			...
			if err != nil {
				return false, fmt.Errorf("failed to generated request: %s", err)
			}
		...
		case <-time.After(task.PollTimeout):
			return false, fmt.Errorf("concord process timedout, after %d seconds", int64(task.PollTimeout/time.Second))
		case <-ctx.Done():
