...
const chronosphereOnlineQuery = "vector(1)"

// ChronosphereProvider executes promQL queries
type ChronosphereProvider struct {
...
// NewChronosphereProvider takes a provider spec and the credentials map,
// validates the address, extracts the bearer token or username and password values if provided and
// returns a Chronosphere client ready to execute queries against the API
func NewChronosphereProvider(provider flaggerv1.MetricTemplateProvider, credentials map[string][]byte) (*ChronosphereProvider, error) {
...
func (p *ChronosphereProvider) RunQuery(query string) (float64, error) {
	query = url.QueryEscape(p.trimQuery(query))
	u, err := url.Parse(fmt.Sprintf("./data/metrics/api/v1/query?query=%s", query))
	if err != nil {
	...
	var result chronosphereResponse
...
// IsOnline run simple Chronosphere query and returns an error if the API is unreachable
func (p *ChronosphereProvider) IsOnline() (bool, error) {
	value, err := p.RunQuery(chronosphereOnlineQuery)
	if err != nil {
		return false, fmt.Errorf("running query failed: %w", err)
	}

	if value != float64(1) {
		return false, fmt.Errorf("value is not 1 for query: %s", chronosphereOnlineQuery)
	}
...
func (p *ChronosphereProvider) trimQuery(query string) string {
