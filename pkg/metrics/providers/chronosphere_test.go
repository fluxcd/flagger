...
type fakeChronosphereClients struct {
	kubeClient    kubernetes.Interface
	flaggerClient clientset.Interface
}

func chronosphereFake() fakeChronosphereClients {
	provider := flaggerv1.MetricTemplateProvider{
		Type:      "chronosphere",
		Address:   "https://fake.chronosphere.io:443",
		SecretRef: &corev1.LocalObjectReference{Name: "chronosphere"},
	}
	...
	return fakeChronosphereClients{
		kubeClient:    kubeClient,
		flaggerClient: flaggerClient,
	}
}

func TestNewChronosphereProvider(t *testing.T) {
	clients := chronosphereFake()

	template, err := clients.flaggerClient.FlaggerV1beta1().MetricTemplates("default").Get(context.TODO(), "chronosphere", metav1.GetOptions{})
	require.NoError(t, err)

	secret, err := clients.kubeClient.CoreV1().Secrets("default").Get(context.TODO(), "chronosphere", metav1.GetOptions{})
	require.NoError(t, err)

	prom, err := NewChronosphereProvider(template.Spec.Provider, secret.Data)
	require.NoError(t, err)

	assert.Equal(t, "https://fake.chronosphere.io:443", prom.url.String())
	assert.Equal(t, "password", prom.password)
}

func TestChronosphereProvider_RunQueryWithBasicAuth(t *testing.T) {
	...
	for _, tt := range noResultTests {
		t.Run(tt.name, func(t *testing.T) {
			...
			template, err := clients.flaggerClient.FlaggerV1beta1().
				MetricTemplates("default").Get(context.TODO(), "chronosphere", metav1.GetOptions{})
			require.NoError(t, err)
			template.Spec.Provider.Address = ts.URL
...
func TestChronosphereProvider_RunQueryWithBearerAuth(t *testing.T) {
	t.Run("ok", func(t *testing.T) {
		expected := `sum(envoy_cluster_upstream_rq)`
		ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			promql := r.URL.Query()["query"][0]
		...
		secret, err := clients.kubeClient.CoreV1().Secrets("default").Get(context.TODO(), "chronosphere-bearer", metav1.GetOptions{})
		require.NoError(t, err)
...
func TestChronosphereProvider_IsOnline(t *testing.T) {
	t.Run("fail", func(t *testing.T) {
		...
		prom, err := NewChronosphereProvider(template.Spec.Provider, nil)
		require.NoError(t, err)
