...
func TestGatewayAPIRouter_getSessionAffinityRouteRules(t *testing.T) {
	...
	weightedRouteRule := &v1.HTTPRouteRule{
		BackendRefs: []v1.HTTPBackendRef{
			router.makeHTTPBackendRef(pSvcName, initialPrimaryWeight, canary.Spec.Service.Port, canary.Spec.Service.PrimaryBackend),
			router.makeHTTPBackendRef(cSvcName, initialCanaryWeight, canary.Spec.Service.Port, canary.Spec.Service.CanaryBackend),
		},
	}
